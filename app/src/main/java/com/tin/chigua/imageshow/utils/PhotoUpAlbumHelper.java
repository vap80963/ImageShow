		...
		if (cur.moveToFirst()) {
			...
			do {
						...
						.replaceAll(" ", "").length()<=0)
				{
					...
					long bucketId = cur.getLong(bucketIdIndex);
...
//					imageItem.setThumbnailPath(thumbnailList.get(_id));
					...
					Log.i(TAG, "PhotoUpAlbumHelper类中 的——》path="+thumbnailList.get(String.valueOf(_id)));
				}
			} while (cur.moveToNext());
		}
		cur.close();
	...
	public List<PhotoUpImageBucket> getImagesBucketList(boolean refresh) {
		...
		insertIntoDatabase(tmpList);
		return tmpList;
	}

	private void insertIntoDatabase(final List<PhotoUpImageBucket> tmpList) {
		new Thread(new Runnable() {
			@Override
			public void run() {
				dbManager = new DBManager(context);
				dbManager.insertToAlbum(tmpList);
			}
		}).start();
	}
